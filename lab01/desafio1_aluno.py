...
Matheus Izidro Campos dos Santos - 12221BCC015
...
class Submissao:
    DIM_MAX = DIM_MAX
    MAX_PARES = 1  # limite fixo (nao cresce com d) p/ nao gerar ruido em alta dimensao

    def fit(self, X: torch.Tensor) -> None:
        """Padroniza (media 0, desvio 1) usando so os dados de treino."""
        self.mu = X.mean(0)
    ...
    def phi(self, X: torch.Tensor) -> torch.Tensor:
        ...
        Combina:
          - termos polinomiais por eixo (linear, quadrado, cubo) -> circulos/esfera/luas
          - produto total das coordenadas -> resolve XOR em 2D
          - geometria radial (R^2, R) -> circulos/esfera (fronteira = limiar de raio)
          - escada de senos/cossenos por eixo, escalados pelo raio -> espiral
          - termos cruzados rotacionados sin(xi+xj), cos(xi-xj) -> fronteiras
            diagonais como XOR/luas, que senos por eixo isolado nao capturam
        O numero de pares cruzados NAO cresce com d, para nao poluir tarefas
        de alta dimensao (ex.: esfera_10d, onde o sinal relevante e so R^2).
        """
        ...
        features = [
            Xs,                                   # dados lineares padronizados
            Xs ** 2,                              # quadrados (circulos, esfera)
            Xs ** 3,                              # cubicos (ajudam nas luas)
            torch.prod(Xs, dim=1, keepdim=True),  # produto total (resolve XOR em 2D)
            R2,                                    # raio ao quadrado (circulos/esfera)
            R                                      # raio linear
        ]

        # escada de frequencias por eixo, escalada pelo raio (boa p/ espiral)
        freq = 1.0
        while True:
            novas_features = [
                torch.sin(Xs * freq),
            ...
            dim_atual = sum(f.shape[1] for f in features)
            dim_novas = sum(f.shape[1] for f in novas_features)

            if dim_atual + dim_novas <= self.DIM_MAX:
                features.extend(novas_features)
                freq += 1.0
            else:
                # usa o espaço restante com termos cruzados rotacionados,
                # que capturam fronteiras diagonais (XOR, luas) — limitado
                # a MAX_PARES pares para nao virar ruido quando d e grande
                pares = [(i, j) for i in range(d) for j in range(i + 1, d)][: self.MAX_PARES]
                extras = []
                for (i, j) in pares:
                    soma, diff = Xs[:, i] + Xs[:, j], Xs[:, i] - Xs[:, j]
                    candidatos = [torch.sin(soma).unsqueeze(1), torch.cos(diff).unsqueeze(1)]
                    custo = sum(c.shape[1] for c in extras) + sum(c.shape[1] for c in candidatos)
                    if dim_atual + custo <= self.DIM_MAX:
                        extras.extend(candidatos)
                    else:
                        break
